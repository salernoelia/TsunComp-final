...
function svg2gcode(svg, settings) {
  ...
  for (
    var pathIdx = 0, pathLength = paths.length;
    pathIdx < pathLength;
    pathIdx++
  ) {
    counter++;
    ...
    // seek to index 0
    gcode.push(
      ["G0", "X" + scale(path[0].x), "Y" + scale(height - path[0].y)].join(" ")
    );
    ...
    if (!settings.LineWithVariationIsDesactivated) {
      ...
      var countZ =
        settings.LineWithVariationMax - settings.LineWithVariationMin;
      var multMaxStrokeWidth =
        MaxStrokeWidth - 1 == 0 ? 0 : countZ / (MaxStrokeWidth - 1);

      if (isSamePath || lastSamePath) {
        var strokeNextPath =
          nextPath != null ? nextPath.node.strokeWidth : strokeWidth;
        var countZContinuo =
          settings.LineWithVariationMax - settings.LineWithVariationMin;
        ...
        var count = 0;
        for (
          var segmentIdx = 0, segmentLength = path.length;
          ...
          if (segmentIdx > 0) {
            var distp = Math.pow(
              Math.pow(path[segmentIdx].x - path[segmentIdx - 1].x, 2) +
                Math.pow(path[segmentIdx].y - path[segmentIdx - 1].y, 2),
              0.5
            );
          ...
          var localSegment = [
            ...
            "Z" +
              (settings.LineWithVariationMin +
                multMaxStrokeWidth * (strokeWidth - 1) +
                count),
          ].join(" ");
