...
const SVGReader = {
    ...
    for (var i = 0; i < parentChilds.length; i++) {
      ...
      if (tag.childNodes && i > 0) {
        if (tag.tagName) {
          ...
          node.strokeWidth =
            tag.attributes[tag.attributes.length - 1].value == null
              ? 1
              : tag.attributes[tag.attributes.length - 1].value;
          MaxStrokeWidth =
            node.strokeWidth > MaxStrokeWidth
              ? node.strokeWidth
              : MaxStrokeWidth;
          ...
          if (tag.attributes) {
            for (var j = 0; j < tag.attributes.length; j++) {
              ...
              if (
                attr.nodeName &&
                attr.nodeValue &&
                this.SVGAttributeMapping[attr.nodeName]
              ) {
                this.SVGAttributeMapping[attr.nodeName](
                  this,
                  node,
          ...
          node.xformToWorld = this.matrixMult(
  ...
  SVGAttributeMapping: {
    ...
    __parseColor: function (val, currentColor) {
      if (val.charAt(0) == "#") {
        if (val.length == 4) val = val.replace(/([^#])/g, "$1$1");
        var a = val
          .slice(1)
          .match(/../g)
          .map(function (i) {
        ...
        else a[3] = Math.max(0, Math.min(1, parseFloat(c)));
        return a;
        ...
        // SMIL is evil, but so are we
        return null;
      } else if (val == "remove") {
        return null;
      } else {
        // unknown value, maybe it's an ICC color
        return val;
      }
    },
  },
  ...
  SVGTagMapping: {
    ...
    __getPolyPath: function (tag) {
      // has transform and style attributes
      var subpath = [];
      var verts = getAttribute(tag, "points")
        .toString()
        .split(/[\s,]+/)
        .map(parseFloat);
      var vertnums = [];
  ...
  addArc: function (
    ...
    var y_ = -sp * dx + cp * dy;
    var r2 =
      (Math.pow(rx * ry, 2) - Math.pow(rx * y_, 2) - Math.pow(ry * x_, 2)) /
    ...
    function recursiveArc(parser, t1, t2, c1, c5, level, tolerance2) {
      ...
      if (
        parser.vertexDistanceSquared(c2, parser.vertexMiddle(c1, c3)) >
        tolerance2
      ) {
      ...
      subpath.push(c3);
      if (
        parser.vertexDistanceSquared(c4, parser.vertexMiddle(c3, c5)) >
        tolerance2
      ) {
